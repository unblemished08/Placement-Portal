...
const StudentDetails = ({ appliedStudents }) => {
  ...
  const handleClick = (student) => {
    navigate("/fullDetails", { state: { student } }); // Pass student details to FullDetails page
  };
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden w-3/4">
        <h1
          className={`text-2xl font-bold text-center py-4 text-white ${
            appliedStudents.length > 0 ? "bg-green-500" : "bg-red-500"
          }`}
        >
          STUDENTS APPLIED
        </h1>
                ...
                {appliedStudents.map((student, index) => (
                  <tr key={index} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 px-4 text-center">
                      <button
                        onClick={() => handleClick(student)} // Pass student to FullDetails
